...
export abstract class Entity {
     ...
     * @param patchData
     */
    abstract patchData(patchData:{[key:string]:any});
     ...
     * @returns {Promise} resolve(json), reject(Request)
     */
    abstract follow(keys):Promise<any>;
 ...
 */
export function extend(entity_name:string) {

    class Class implements Entity {
        ...
        private create():Promise <any> {
            return new Promise((resolve, reject) => {
                Class.translateRelationEntity(this.data()).then(body=> {
        ...
        private update():Promise<any> {
            return new Promise((resolve, reject)=> {
                ...
                Class.translateRelationEntity(pureChange).then((json)=> {
        ...
        fetch():Promise<any> {
            return new Promise((resole, reject) => {
                Class.findOne(this.id).then(entity => {
         ...
         * @param json
         */
        static jsonToEntityList(json:{[key:string]:any}):Class[] {
            let re = [];
            let arr:any[] = json['_embedded'][this.entityName];
            arr.forEach(one=> re.push(new Class(one)));
            return re;
        }
        ...
        private static translateRelationEntity(data:any):Promise<any> {
            return new Promise((resolve, reject)=> {
                if (isEntity(data)) {//is a Class instance
                    <Class>data.save().then(()=> {
                        resolve(data.href());
                    }).catch((err)=> {
                        reject(err);
                    })
